...
//        [self setErrors:[NSMutableArray new]];
        [self setErrors:[NSMutableArray arrayWithCapacity:13]]; // 10개 제한인데 여류로 3개 더 줌..
...
-(void)addError:(NSError*)error
{
    if (error == nil || error.code == 0)
        return;
    DDLogError(@"Selenium Error: %ld - %@", error.code, error.description);
    [self setLastError:error];
    
    // err 데이터가 무한정 쌓임.. 제한없는 Array에 넣기만 하고 있어서, 10개로 제한을 둠..
    if( 10 == _errors.count ) {
...
-(SESession*) postSessionWithDesiredCapabilities:(SECapabilities*)desiredCapabilities andRequiredCapabilities:(SECapabilities*)requiredCapabilities error:(NSError**)error
{
	...
	NSDictionary *json = [SEUtility performPostRequestToUrl:urlString postParams:postDictionary timeoutInterval:60 error:error];
...
-(void) postTimeout:(NSInteger)timeoutInMilliseconds forType:(SETimeoutType)type session:(NSString*)sessionId error:(NSError**)error
{
    ...
    NSDictionary *postDictionary = [[NSDictionary alloc] initWithObjectsAndKeys: timeoutType, @"type", [NSNumber numberWithInteger:timeoutInMilliseconds], @"ms", nil];
	[SEUtility performPostRequestToUrl:urlString postParams:postDictionary error:error];
}
...
- (void) postVolume:(NSString *)volume session:(NSString *)sessionId error:(NSError **)error
{
    NSString *urlString = [NSString stringWithFormat:@"%@/session/%@/volume", self.httpCommandExecutor, sessionId];
    NSDictionary * postParams = [[NSDictionary alloc] initWithObjectsAndKeys:volume, @"volume", nil];
    [SEUtility performPostRequestToUrl:urlString postParams:postParams error:error];
}

- (void) postPinch:(NSDictionary *)pinchDatas session:(NSString *)sessionId error:(NSError **)error
{
    NSString * urlString = [NSString stringWithFormat:@"%@/session/%@/pinch", self.httpCommandExecutor, sessionId];
    [SEUtility performPostRequestToUrl:urlString postParams:pinchDatas error:error];
}
...
{
    NSString *urlString = [NSString stringWithFormat:@"%@/session/%@/appium/app/end_test_ ", self.httpCommandExecutor, sessionId];
    [SEUtility performPostRequestToUrl:urlString postParams:nil error:error];
}
...
//    NSMutableDictionary *postParams = [NSMutableDictionary dictionaryWithObjectsAndKeys:value, @"text", nil];
    NSMutableDictionary *postParams = [NSMutableDictionary dictionaryWithObjectsAndKeys:value, @"value", nil];
    if (isUnicode) {
