...
{
    __block NSTimer * executeTimeout = nil;
    dispatch_sync(dispatch_get_main_queue(), ^{
        executeTimeout = [NSTimer scheduledTimerWithTimeInterval:TIMEOUT target:self selector:@selector(executeTimeoutTimer:) userInfo:nil repeats:NO];
    });
    
    NSDictionary * result = [self executeScript:script arguments:nil];
    
    if (executeTimeout.valid) {
        [executeTimeout invalidate];
        executeTimeout = nil;
    }
...
-(NSDictionary*) executeScript:(NSString*)script arguments:(NSArray*)arguments error:(NSError**)error
{
    ...
    NSDictionary * result = [self.jsonWireClient postExecuteScript:script arguments:arguments session:self.session.sessionId error:error];
...
-(NSDictionary*) executeAnsynchronousScript:(NSString*)script
{
    ...
    NSDictionary * result = [self executeAnsynchronousScript:script arguments:nil];
...
-(NSDictionary*) executeAnsynchronousScript:(NSString*)script arguments:(NSArray*)arguments error:(NSError**)error
{
    ...
    NSDictionary * result = [self.jsonWireClient postExecuteAsyncScript:script arguments:arguments session:self.session.sessionId error:error];
...
{
    NSError * error;
    [self setVolume:volume error:&error];
    [self addError:error];
}

- (void) setVolume:(NSString *)volume error:(NSError **)error
{
    [self.jsonWireClient postVolume:volume session:self.session.sessionId error:error];
}


- (void) pinch:(NSDictionary *)pinchDatas {
    NSError * error;
    [self pinch:pinchDatas error:&error];
    [self addError:error];
}

- (void) pinch:(NSDictionary *)pinchDatas error:(NSError **)error {
    [self.jsonWireClient postPinch:pinchDatas session:self.session.sessionId error:error];
}
...
#pragma mark - Timer
- (void)executeTimeoutTimer:(NSTimer *)theTimer {
    if( !theTimer.valid )
        return ;
    
    if( _customDelegate && [_customDelegate respondsToSelector:@selector(onTimeOutOfExecScript)] ) {
        [_customDelegate onTimeOutOfExecScript];
    }
