        ...
        dispatch_async(asyncURLConnectionQueue, ^(void) {                                                               // 따라서 Http Module 을 호출한 객체가 소멸되어도 별도로 동작을 하기 때문에 안전함..
            @autoreleasepool {
                ...
                if([cpu isEqualToString:@"x86_64"]){
                    [installTaks setLaunchPath: @"/usr/local/bin/ios-deploy"];
                }else{
                    [installTaks setLaunchPath: @"/opt/homebrew/bin/ios-deploy"];
                }
                

                [installTaks setArguments: @[@"-b", appPath, @"-i", udid]];
...
//                }
                ...
                if (@available(macOS 10.13, *)) {
                    BOOL bSuccess = [installTaks launchAndReturnError:&error];
                    if(bSuccess){
                        NSLog(@"설치 성공");
                    }else{
                        NSLog(@"설치 실패");
                    ...
                    if(error){
                        NSLog(@"install error = %@",error.description);
                        NSString* result = [NSString stringWithFormat:@"앱 설치오류"];
                        block(NO, result);
...
//
                ...
                if (output == nil || [output isEqualToString:@""] || [output isEqual:[NSNull null]] || [output hasPrefix:@"ERROR:"] || arrOut == nil) {
    ...
    //                        [[CommunicatorWithDC sharedDCInterface] commonResponseForInstall:NO appId:result deviceNo:_deviceInfos.deviceNo];
                        ...
                        else if ([tmp hasSuffix:@"[100%]"]) {
                            // 어플 설치 성공.
                            DDLogWarn(@"install success");
                            block(YES, nil);
                            return ;
                        }else if([tmp containsString:@"[100%]"]){
                            DDLogWarn(@"install success");
                            block(YES, nil);
                            return;
                        }
    ...
    dispatch_async(asyncURLConnectionQueue, ^(void) {                                                               // 따라서 Http Module 을 호출한 객체가 소멸되어도 별도로 동작을 하기 때문에 안전함..
        @autoreleasepool {
            ...
            if([cpu isEqualToString:@"x86_64"]){
                ...
                installTaks.launchPath = @"/opt/homebrew/bin/ios-deploy";
                    installTaks.arguments = @[@"-b",
                                          appPath,
                                          @"-i",
                                          udid
                                            ];
